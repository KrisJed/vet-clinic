...
import { UserModel, UserModelStatic } from './users';
...
const getUsers = async (req: Request, res: Response): Promise<void> => {
  try {
    const users: UserModel[] = await db.users.findAll({ raw: true });
    const animals: AnimalModel[] = await db.animals.findAll({ raw: true });

    const data: ExtendedUserModel[] = users.map(
      (user: UserModel): ExtendedUserModel => {
        const userAnimals: AnimalModel[] = animals.filter(
          (animal: AnimalModel): boolean => animal.ownerId === user.id,
        );
...
const getUser = async (req: Request, res: Response): Promise<void> => {
  try {
    let seekId: string = req.params.id;
...
const getUserByAnimal = async (req, res) => {
  try {
    ...
    const user = await db.users.findAll({ raw: true });
    if (user === null) {
      res.status(404);
